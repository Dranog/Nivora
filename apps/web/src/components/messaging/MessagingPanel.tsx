...
import { FanProfileCard } from '@/components/profile/FanProfileCard';
...
export function MessagingPanel({
  ...
  const [showTipDialog, setShowTipDialog] = useState(false); // ✅ État pour le dialog de tip
  ...
  // ✅ Guard: Vérifier que currentUser existe
  if (!currentUser || !currentUser.id) {
    return (
      <div className="my-8 bg-white rounded-2xl shadow-2xl overflow-hidden border border-gray-200 relative p-8">
        <button
          onClick={onClose}
        ...
        <div className="text-center">
          <p className="text-xl font-semibold text-gray-900 mb-2">Erreur de chargement</p>
  ...
  const sendMessage = () => {
    ...
    // ✅ Guard: Vérifier que currentUser existe
    if (!currentUser || !currentUser.id) {
      console.error('Cannot send message: currentUser is undefined');
    ...
    const newMessage: Message = {
      ...
      senderAvatar: currentUser.avatar || 'https://i.pravatar.cc/150?img=1',
  ...
  // ✅ VERSION SÉCURISÉE avec guards et i18n
  const formatTime = (date: Date | string | undefined | null): string => {
    // Guard: si date invalide, retourner fallback
    if (!date) {
      return t('messages.time.justNow');
    }

    // Conversion en Date si string
    ...
    if (minutes < 1) {
      ...
      return t('messages.time.minutesAgo', { count: minutes });
    } else if (hours < 24) {
      return t('messages.time.hoursAgo', { count: hours });
    } else if (days === 1) {
      return t('messages.time.yesterday');
    } else {
      return t('messages.time.daysAgo', { count: days });
    }
  };
    ...
    <div className="my-8 bg-white rounded-2xl shadow-2xl overflow-hidden border border-gray-200 relative">
            ...
            {currentUser && currentUser.username && (
              <FanProfileCard
                fan={{
                  ...
                  subscriptionsCount: currentUser.subscriptionsCount,
                }}
                className="mb-4"
              />
            )}
            ...
            <>
              ...
              <div className="p-4 bg-white border-b border-gray-200 flex items-center justify-between flex-shrink-0">
                ...
                <div className="flex items-center gap-3">
                  ...
                  {/* ✅ Bouton Video conditionnel */}
                  {activeConversation.allowsVideoCalls && (
                    <Button
                      variant="ghost"
                      size="icon"
      ...
      {activeConversation && (
        <TipDialog
          ...
          creatorId={activeConversation.participantId}
          creatorName={activeConversation.participantName}
        />
      )}
    </div>
  );
